### Prelim regression with specific ethnicities
...
msds <- read_excel("../../data/MSDS/MSDSv1_processed_unknowns.xlsx") %>%
  select(-c("ApgarScore5", "BroadEthnicityMother")) %>%
  mutate("Premature or LBW" = Premature | LowBirthWeight,
         EthnicDescriptionMother = case_when(
           EthnicDescriptionMother == "NULL" ~ "Unknown",
           EthnicDescriptionMother == "Not known" ~ "Unknown",
           EthnicDescriptionMother == "Not stated" ~ "Unknown",
           TRUE ~ EthnicDescriptionMother
         ))
...
msds$EthnicDescriptionMother <- relevel(msds$EthnicDescriptionMother, ref = "British")
...
for (set in c("Premature", "Low birth weight", "Both")) {
  ...
  dataset_names[set][[1]]["Variable group"][dataset_names[set][[1]]["Variable group"] == "EthnicDescriptionMother"] <- "Mother Eth"
...
openxlsx::write.xlsx(dataset_names, 
                     file = '../../data/outputs/prelim_reg_results_fullEth.xlsx') 