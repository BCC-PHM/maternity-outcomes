...
MSDS <- read_excel("../../data/MSDS/MSDS_v1.xlsx") %>%
  ...
  mutate(
    ...
    PhysicalDisabilityStatusIndMother = case_when(
      PhysicalDisabilityStatusIndMother == "Y" ~ TRUE,
      PhysicalDisabilityStatusIndMother == "N" ~ FALSE,
      TRUE ~ NA),
    FirstLanguageEnglishIndMother = case_when(
      FirstLanguageEnglishIndMother == "Y" ~ TRUE,
      FirstLanguageEnglishIndMother == "N" ~ FALSE,
      TRUE ~ NA),
    late_booking = case_when(
      late_booking == "1" ~ TRUE,
      late_booking == "0" ~ FALSE,
      TRUE ~ NA),
    MentalHealthIssues = case_when(
      MHPredictionDetectionIndMother == "Y" ~ TRUE,
      MHPredictionDetectionIndMother == "N" ~ FALSE,
      TRUE ~ NA),
    ComplexSocialFactors = case_when(
      ComplexSocialFactorsInd == "Y" ~ TRUE,
      ComplexSocialFactorsInd == "N" ~ FALSE,
      TRUE ~ NA),
    ...
    PreviousLiveBirths = case_when(
      PreviousLiveBirths == 0 ~ FALSE,
      PreviousLiveBirths > 0 ~ TRUE,
      TRUE ~ NA),
    PreviousCaesareanSections = case_when(
      PreviousCaesareanSections == 0 ~ FALSE,
      PreviousCaesareanSections > 0 ~ TRUE,
      TRUE ~ NA),
    PreviousStillBirths = case_when(
      PreviousStillBirths == 0 ~ FALSE,
      PreviousStillBirths > 0 ~ TRUE,
      TRUE ~ NA),
    PreviousLossesLessThan24Weeks = case_when(
      PreviousLossesLessThan24Weeks == 0 ~ FALSE,
      PreviousLossesLessThan24Weeks > 0 ~ TRUE,
      TRUE ~ NA),
    AnE_visits_pre_con = case_when(
      AnE_visits_pre_con == 0 ~ FALSE,
      AnE_visits_pre_con > 0 ~ TRUE,
      TRUE ~ NA),
    AnE_visits_early_preg = case_when(
      AnE_visits_early_preg == 0 ~ FALSE,
      AnE_visits_early_preg > 0 ~ TRUE,
      TRUE ~ NA),
    AnE_visits_preg = case_when(
      AnE_visits_preg == 0 ~ FALSE,
      AnE_visits_preg > 0 ~ TRUE,
      TRUE ~ NA),
    AlcoholAtBooking = case_when(
      AlcoholUnitsPerWeek == "0" ~ FALSE,
      AlcoholUnitsPerWeek == "NULL" ~ NA,
      TRUE ~ TRUE),
...
write_xlsx(MSDS_reduced, "../../data/MSDSv1_processed.xlsx")
