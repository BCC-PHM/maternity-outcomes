...
setwd("~/Main work/Maternity outcomes/maternity-outcomes/R/statistical_analysis/BadgerNet")
...
# limit ref to relevant variables
ref <- ref %>%
...
# all_important <- c(
#   "NumberOfBabies", "substanceAbuse","Social Services",
#   "DomesticAbuse","MentalHealth","HousingIssues",
#   "FinancialIssues","sensoryAndPhysicalDis","BMI>35",
#   "IMD Quintile","Ethnicity","SmokingAtDelivery",
#   "Gestational_Diabetes","Consanguineous Union",
...
less_variables <- c("Ethnicity Group","IMD Quintile", "Age Group",  "Folic Acid Taken", 
           "Financial/Housing Issues", "Twins", "Smoking at Delivery",
           "Mental Health Issue(s)")

formula_minus <- write_formula("`Output:`", all_important, quote_wrap = TRUE)
```

Our maximused formula now becomes:
...
fplot <- function(model_tib, 
  ...
  model_tib <- tidyr::separate(model_tib, col = `**95% CI**`, into = c("lowerCI", "upperCI"), sep = ",")
  model_tib <- model_tib %>%
  mutate(
  ...
  # Prepare forest plot
  new_fp <- model_tib %>% 
    select(c("**Characteristic**", "Odds Label", "**p-value**" )) |> 
    forestplot(
    clip = c(-1, 6),
    graph.pos=2,
...
lbw_fp <- fplot(lbw_exp, title = "Low Birth Weight")
prem_fp <- fplot(prem_exp, title = "Premature")
...
ggplot2::ggsave(both_big, 
                filename = "../../../outputs/figures/reg_lbw_prem.pdf", 
                width = 12,
                height = 14)
```
...
sb_fp <- fplot(sb_exp, title = "Stillbirth")
nd_fp <- fplot(nd_exp, title = "Neonatal Death")
...
ggplot2::ggsave(both_smalls, 
                filename = "../../../outputs/figures/reg_sb_nd.pdf", 
                width = 12,
                height = 7)
```